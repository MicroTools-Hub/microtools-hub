...
export default function FacebookDownloader() {
  ...
  const fetchVideo = async ()=> {
    setLoading(true);
    const res = await fetch(`${BACKEND}/api/facebook`, { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({url}) });
    const data = await res.json(); setInfo(data); setLoading(false);
  };

  return (
    <>
      <SEO title="Facebook Video Downloader — MicroTools Hub" description="Download Facebook videos fast" />
      <div className="max-w-3xl mx-auto p-4">
        <h1 className="text-3xl font-bold text-indigo-600 mb-6">Facebook Video Downloader</h1>
